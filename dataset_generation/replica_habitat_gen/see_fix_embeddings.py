...
if __name__ == '__main__':
    ...
    for sid in tqdm.tqdm(list(range(48))):
        ...
        c2v_fix = npz_fix[f'center_to_vertex_{sid:02d}']
        cpts_fix = npz_fix[f'center_point_{sid:02d}']

        # print(sid, np.max(np.abs(cpts_fix - cpts)))
        # print(sid, np.max(np.abs(c2v_fix - c2v)))
        # print()

        if sid in val_sid:
            pass
        else:
            train_pts.append(fts[::10])
            train_fix_pts.append(fts_fix[::10])
