...
if __name__ == '__main__':
    ...
    for sid in range(a, b):
        ...
        for k, i, j in tqdm.tqdm(triplets):
            ...
            already = glob.glob(f'ReplicaGenEncFtTriplets/{stage}/npz_extrap/{sid:02d}_{k:03d}*.npz')
            if len(already) > 0:
                tar = f'ReplicaGenEncFtTriplets/{stage}/npz_extrap/{basename}.npz'
                os.system(f'cp {already[0]} {tar}')
                continue

            idx_in, _ = load(f'{sid:02d}_{k:03d}', 'npz_singlecrop')
            ...
            val_in = output_info.features_at_coordinates(idx_in.float())

            input_info = ME.SparseTensor(
                coordinates=torch.cat([idx_in, idx_out], dim=0),
                features=torch.cat([val_in, val_out*0], dim=0),
                coordinate_manager=output_info.coordinate_manager,
                quantization_mode=ME.SparseTensorQuantizationMode.UNWEIGHTED_SUM,
            )

            assert(input_info.C.shape == output_info.C.shape)
            output_ft = output_info.features_at_coordinates(input_info.C.float())
            if input_info.C.shape[0] == idx_out.shape[0] and torch.round(1-output_ft[:,-1]).int().sum().item() == 0:
                pass
            else:
                print(basename)
            ...
            #
            np.savez_compressed(
                f'ReplicaGenEncFtTriplets/{stage}/npz_extrap/{basename}.npz',
