...
def rotmat2pose(rotmat):
    # rotmat: N x 4 x 4
    neg_yz = np.diag([1.0,-1.0,-1.0,1.0]).astype(np.float32)
    rotmat = rotmat.astype(np.float32).dot(neg_yz)
    rrr, ttt = rotmat[:,:3,:3], rotmat[:,:3,3] # N x 3
    rrr = R.from_matrix(rrr).as_quat() # N x 4
    return np.concatenate([ttt, rrr[:,[3,0,1,2]]], axis=1)
...
def project_depth_dist(pts, int_mat, ext_mat):
    assert(len(pts.shape) == 2) # (N, 3)
    local = np.concatenate([pts, np.ones((pts.shape[0], 1), np.float32)], axis=-1)
    local = local.dot(np.linalg.inv(ext_mat).T)[:, :3]
    local = local.dot(int_mat.T)
    dist = np.sqrt((local ** 2).sum(axis=-1))
    local[:, :2] /= local[:, 2:]
    return local[:, :2], local[:, 2], dist
