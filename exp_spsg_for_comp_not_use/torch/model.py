...
class GeoGeneratorTDF(nn.Module):
    def __init__(self, nf_in_geo, nf, max_data_size, max_dilation=1):
        nn.Module.__init__(self)
        self.data_dim = 3
        self.nf = nf
        self.max_data_size = np.array(max_data_size)
    ...
    def forward(self, x_geo, mask=None):
        # x_geo: B C x y z        
        # if mask is not None:
        #     x = torch.cat([x_geo, mask], 1)
        #     x_geo = x[:,:1,:,:,:]
        #     mask = x[:,4:,:,:,:]
        # else:
        #     x_geo = x_geo[:,:1,:,:,:]
...
class GeoGenerator(nn.Module):
    def __init__(self, nf_in_geo, nf, max_data_size, max_dilation=1):
        ...
        num_params_geo = count_num_model_params(self.geo_0) + count_num_model_params(self.geo_1) + count_num_model_params(self.geo_2) + count_num_model_params(self.geo_occ)
        print('#params(geo) = ', num_params_geo)
    ...
    def forward(self, x_geo):     
        ...
        x_geo = self.geo_0(x_geo) # downsample
        # print(0, x_geo.shape)
        x_geo = self.geo_1(x_geo) # downsample
        # print(1, x_geo.shape)
        x_geo = torch.nn.functional.interpolate(x_geo, scale_factor=scale_factor, mode=self.interpolate_mode) # upsample
        # print('ii', x_geo.shape)
        x_geo = self.geo_2(x_geo) # shape not change
        # print(2, x_geo.shape)
        x_geo = torch.nn.functional.interpolate(x_geo, scale_factor=scale_factor, mode=self.interpolate_mode) # upsample
        # print('ii', x_geo.shape)
        x_occ = self.geo_occ(x_geo) # shape not change
        # print('occ', x_occ.shape)
        
        return x_occ


class GeoGeneratorDF(GeoGenerator):

    def __init__(self, nf_in_geo, nf, max_data_size, max_dilation=1):
        super().__init__(nf_in_geo, nf, max_data_size, max_dilation)
        nz_in = max_data_size[0]
        kz = [1] * 34 if nz_in == 1 else [5, 4,3, 4,3,3, 3,3,3,3,3, 3, 3,3, 3,3,3, 5, 4,4, 3,3,3, 3,3,3,3, 3,3, 3,3, 3,3,3]
        self.geo_3 = torch.nn.Sequential(
            torch.nn.Conv3d(self.nf, self.nf//2, 3, 1, 1, bias=self.use_bias),
...
class GeoGeneratorDFv2(GeoGenerator):
