...
rgb_dst_dir = 'res_3query_replica_gt_sltd/rgb'
gif_dst_dir = 'res_3query_replica_gt_sltd/gif'
...
for sltd_3query in tqdm.tqdm(sltd_3queries):
    ...
    for i, view_name in enumerate(view_names):
        ...
        view_idx = all_val_views.index(f'{s}_{view_name}')
        view_img_indices = list(range(view_idx*15, view_idx*15+15))
        if orders[i] == -1:
            view_img_indices = view_img_indices[::-1]
        # New
        view_imgs = [Image.open(f'{src_gt_dir}/{img_idx:05d}.png').convert('RGB') for img_idx in view_img_indices]
        # view_imgs = [Image.open(f'{src_gt_dir}/output{s}/color/{img_idx:05d}.png').convert('RGB') for img_idx in view_img_indices]
        sltd_3query_imgs += view_imgs[:14]
...
# src_gt_dir = '/home/lzq/lzy/NSVF/ARKitScenesTripletsNSVF/all/mink_nsvf_rgba_sep_field_base/output_video'
...
#     view_img_indices = list(range(view_idx*15, view_idx*15+15))
#     # New
#     view_imgs = [Image.open(f'{src_gt_dir}/color/{img_idx:05d}.png').convert('RGB').rotate(int(angle), expand=1) for img_idx in view_img_indices]
