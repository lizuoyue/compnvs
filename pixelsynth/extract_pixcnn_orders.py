...
if __name__ == "__main__":
    ...
    MODEL_PATH = test_ops.old_model # log/mp3d/pretrained_dpr/model_epoch.pth
    BATCH_SIZE = test_ops.batch_size    # 16
    
    opts = torch.load(MODEL_PATH)["opts"]
    opts.config = 'habitat-lab/configs/tasks/pointnav_rgbd.yaml'
    opts.dataset_folder = test_ops.dataset_folder   # /x/cnris/mp3d_vqvae/train
    opts.dataset = test_ops.dataset     # custom_mp3d
    opts.model_setting = test_ops.model_setting     # get_gen_order
    opts.batch_size = test_ops.batch_size   # 16

    Dataset = get_dataset(opts)
    data = Dataset("test", opts)
    model = get_model(opts)     # from models.z_buffermodel import ZbufferModelPts
    ...
    if "sync" in opts.norm_G:   # norm_G = 'sync:spectral_batch'
