...
def get_args():
    ...
    parser.add_argument("--load_ckpt", type=str, default="ckpt_comp_w1/ckpt_1_0.pt")
...
class ReplicaGeoCompDataset(Dataset):
    ...
    def __init__(self, voxel_size, triplets_folder, npz_folder, scene_ids):
        ...
        for sid in scene_ids:
            triplets = np.load(f'{triplets_folder}/scene{sid:02d}_triplet_idx.npz')['mid']#['easy']
    ...
    def load_npz(self, sid, fid):
        return np.load(f'{self.npz_folder}/{sid:02d}_{fid:03d}.npz')['pts']
...
def forward(net, data, loss_fn, save=None):
    ...
    if save is not None:
        ...
        for idx, pc_pd in enumerate(li_pd):
            ...
            np.savez_compressed(f'{save}/{sid:02d}_{k:03d}_{i:03d}_{j:03d}.npz', pred_geo=pc_pd.cpu().numpy())
    ...
    for epoch in range(1, args.epoch):
        ...
        for it, data_dict in enumerate(tqdm.tqdm(train_iter)):
            ...
            if it % 500 == 0:# and it != 0:
                ...
                torch.save(d, f'ckpt_compfull_w3/ckpt_{epoch}_{it}.pt')
...
def test_model(net, data_loader, args):
    ...
    for it, data_dict in enumerate(tqdm.tqdm(data_iter)):
        with torch.no_grad():
            loss, losses, ious, sout = forward(net, data_dict, loss_fn, save=f'mink_comp_pred_w1_mid')
...
if __name__ == '__main__':
    ...
    train_data = ReplicaGeoCompDataset(voxel_size, triplets_folder, npz_folder, train_sid[args.start_from: args.start_from+6])
    ...
    val_data_loader = DataLoader(val_data, batch_size=8, shuffle=False, collate_fn=collate_fn)
