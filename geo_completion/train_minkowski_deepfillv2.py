...
from minkowski_network import MinkowskiDeepFillv2Discriminator
import MinkowskiEngine as ME
from utils import replace_features
from minkowski_loss_fn import minkowski_masked_l1_loss
from minkowski_loss_fn import minkowski_hinge_loss
...
def val_model(
    ...
    # Forward
    with torch.no_grad():
        fake_y_coarse, fake_y = gen(real_x)

        pred_fake = dis(ME.cat(fake_y, mask_fill))
        loss_dis_fake = dis_loss_fn(pred_fake, 0)

        pred_real = dis(ME.cat(real_y, mask_fill))
        loss_dis_real = dis_loss_fn(pred_real, 1)

        loss_dis = (loss_dis_fake + loss_dis_real) * 0.5

        pred_fake = dis(ME.cat(fake_y, mask_fill))
        loss_gen_gan = dis_loss_fn(pred_fake, -1)

        loss_gen_err = gen_loss_fn(fake_y, real_y, mask_fill) + gen_loss_fn(fake_y_coarse, real_y, mask_fill)
...
def train_model(
    ...
    for epoch in range(100):
        for it, data in enumerate(tqdm.tqdm(data_loader)):
            ...
            # Forward
            fake_y_coarse, fake_y = gen(real_x) # G(x)
...
if __name__ == '__main__':
    ...
    npz_folder = '../replica_habitat_gen/ReplicaGenFtTriplets/easy/npz_regdim32emb32out'
    ...
    device = 'cuda:0'
    emb_size, mid_size = 32, 64
    gen = MinkowskiDeepFillv2(emb_size + 1, emb_size, mid_size).to(device)
    dis = MinkowskiDeepFillv2Discriminator(emb_size + 1, mid_size).to(device)
    
    gen_criterion = minkowski_masked_l1_loss
    gen_optimizer = torch.optim.Adam(gen.parameters(), lr=1e-4)

    dis_criterion = minkowski_hinge_loss
    dis_optimizer = torch.optim.Adam(dis.parameters(), lr=1e-4)

    save_path = 'ckpt_minkowski_deepfillv2_'
