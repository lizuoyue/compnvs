...
from minkowski_pconv_unet import MinkUNet50
...
def val_model(
    ...
    real_x = ME.SparseTensor(coordinates=vertex_idx, features=vertex_input[:,:-1])
...
def train_model(
    ...
    for epoch in range(epoch_saved, 100):
        ...
        for it, data in enumerate(tqdm.tqdm(data_loader)):
            ...
            # Forward
            fake_y, _ = gen((real_x, mask_keep)) # G(x)
            fake_y += real_x
            # fake_y = replace_features(fake_y, fake_y.F * mask_fill.F + real_y.F * mask_keep.F)
...
if __name__ == '__main__':
    ...
    train_data = ReplicaGenDataset('../replica_habitat_gen/ReplicaGenEncFtTriplets/easy/npz', train_sid)
    val_data = ReplicaGenDataset('../replica_habitat_gen/ReplicaGenEncFtTriplets/easy/npz', val_sid)
    ...
    gen = MinkUNet50(emb_size, emb_size).to(device)
    ...
    save_path = 'ckpt_minkowski_pconv_unet50_encft'
    train_model(
