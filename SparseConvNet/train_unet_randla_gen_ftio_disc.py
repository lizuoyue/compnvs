...
def train_model(data_loader, 
    ...
    ckpt_savefile = 'ckpt_unet_randlanet_gen_ftio'
    ...
    for epoch in range(100):
        for it, data in enumerate(tqdm.tqdm(data_loader)):
            ...
            mask = (1 - vertex_input[..., -1]).bool() # mask_fill
...
class TwoNet(nn.Module):
    ...
    def __init__(self, scn, randlanet):
        self.scn = scn
...
if __name__ == '__main__':
    ...
    val_dataloader = DataLoader(val_data, batch_size=1, shuffle=True, collate_fn=collate_fn)

    device = 'cuda:0'
    scn_unet = SparseConvUNet(33, 32, torch.Tensor([144, 64, 160]).long(), reps=2, n_planes=[i * 32 for i in [1, 2, 3, 4, 5]]).to(device)
    randlanet = RandLANet(33, 32, device=torch.device(device))
    scn_unet = nn.Sequential(*[scn_unet, randlanet])
    disc = Discriminator(33).to(device)
