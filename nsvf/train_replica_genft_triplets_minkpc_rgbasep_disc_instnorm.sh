...
CUDA_VISIBLE_DEVICES=5 python3 train.py ${DATASET} \
    ...
    --train-views "glob_ReplicaGenEncFtTriplets_easy_train_full" \
    --view-resolution $RES \
    --max-sentences 1 \
    --view-per-batch 1 \
    --backbone-name 'MinkUNet50_inst' \
    --pixel-per-view 2048 \
    --no-preload \
    --use-discriminator \
    --sampling-on-mask 1.0 --no-sampling-at-reader \
    --valid-view-resolution $RES \
    --valid-views "glob_ReplicaGenEncFtTriplets_easy_trainval_tiny" \
    ...
    --total-num-update 300001 \
    --lr 0.0005 \
    --clip-norm 0.0 \
    --criterion "srn_loss" \
    --num-workers 0 \
    --seed 2 \
    --save-interval-updates 200 --max-update 300000 \
