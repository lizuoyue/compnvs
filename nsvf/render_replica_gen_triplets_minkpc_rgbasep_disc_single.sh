...
MODEL=$ARCH$SUFFIX
MODEL_PATH=$SAVE/$MODEL/checkpoint77.pt
...
CUDA_VISIBLE_DEVICES=3 python3 render.py ${DATASET} \
    ...
    --render-views "glob_ReplicaGenEncFtTriplets_easy_val_npz_full" \
    --test-views "glob_ReplicaGenEncFtTriplets_easy_val_npz_full" \
    --model-overrides $MODELARGS \
    --render-resolution $RES \
    --render-output ${SAVE}/$ARCH/output0306 \
