...
class ImageReader(Reader):
    ...
    def sample_pixels(self, uv, size, alpha=None, mask=None, **kwargs):
        ...
        # import numpy as np
        # Image.fromarray((mask[0,0].int().cpu().numpy()*255).astype(np.uint8)).save('aaa_mask.png')
        # assert(False)
        # quit()
        ...
        # Image.fromarray(((probs[0,0]>0).int().cpu().numpy()*255).astype(np.uint8)).save('aabb.png')
        ...
        # Image.fromarray(((sampled_masks[0,0]>0).int().cpu().numpy()*255).astype(np.uint8)).save('bbcc.png')
