...
class NeuralRenderer(object):
    ...
    def __init__(self, 
        ...
        if test_camera_poses is not None:
            ...
            elif os.path.isdir('/'.join(test_camera_poses.split('/')[:-1])):
                self.test_poses = [
                    # np.load(f)['poses'][7:8] for f in sorted(glob.glob(test_camera_poses + "*.npz"))
    ...
    @torch.no_grad()    
    def generate_single(self, model, sample, step, frames):

        assert('vertex_idx' in sample)
        output_path = self.output_dir
        image_names = []
        ...
        for shape in range(sample['shape'].size(0)):
            ...
            _sample = {
                ...
                'colors': torch.cat([real_images[shape:shape+1] for _ in range(step, next_step)], 1),
                'intrinsics': sample['intrinsics'][shape:shape+1],
                'extrinsics': torch.stack(inv_RT, 0).unsqueeze(0), # shape 1, N, 4, 4
                'uv': torch.stack(uv, 0).unsqueeze(0),
                'shape': sample['shape'][shape:shape+1],
                'view': torch.arange(
                    step, next_step, 
            ...
            for keyword in keywords:
                if keyword in sample:
                    _sample[keyword] = sample[keyword][shape:shape+1]
            ...
            for k in range(step, next_step):
                images = model.visualize(_sample, None, 0, k-step)
                image_name = "{:05d}".format(k)

                if 'zuoyue_depth' in images:
                    org_depth = images.pop('zuoyue_depth')
                ...
                for key in images:
                    ...
                    if type in self.output_type:
                        ...
                        if type == 'point':
                            ...
                            if type == 'depth':
                                dep_path = os.path.join(prefix, image_name + '.npz')
                                np.savez_compressed(dep_path, org_depth.cpu().numpy())
                            image = images[key].permute(2, 0, 1) \
                                if images[key].dim() == 3 else torch.stack(3*[images[key]], 0)        
                            save_image(image, os.path.join(prefix, image_name + '.png'), format=None)
                            image_names.append(os.path.join(prefix, image_name + '.png'))
