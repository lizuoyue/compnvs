...
def make_mlp(num_layers, num_channels, d_in, d_out):
    layers = [
        nn.Linear(d_in, num_channels),
        nn.ReLU()
    ]
    ...
    layers += [
        nn.Linear(num_channels, d_out, bias=False)
    ]
...
if __name__ == '__main__':
    ...
    model = make_mlp(16, 256, 256, 32).cuda()
    criterion = nn.L1Loss()
    ...
    for it in range(10000):
        ...
        if it % 100 == 0:
            np.savez_compressed(f'{it:04d}_mlp.npz', dip=pred.detach().cpu().numpy())
