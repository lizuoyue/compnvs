...
class SingleObjRenderingTask(FairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument("--load-npz", type=str, default=None,
                            help="load .npz files if exists")
        parser.add_argument("--fix-field", action="store_true", 
                            help="fix the field part during training")
        parser.add_argument("--fix-embedding", action="store_true", 
                            help="fix the embedding part during training")
        parser.add_argument("--embedding-init", type=str, default='normal',
                            help="TODO")
        parser.add_argument("--dim-compressed", type=int, default=4,
                            help="the number of channels before SCN")
        parser.add_argument("--reg-net", type=str, default="scn",
                            help="the number of channels before SCN")
    ...
    def load_dataset(self, split, **kwargs):
        ...
        self.datasets[split] = ShapeViewDataset(
            ...
            ids=self.object_ids,
            load_npz=self.args.load_npz,
            use_disc=getattr(self.args, "use_discriminator", False),
        )
        ...
        if split == 'test':# and self.args.load_npz:
            self.datasets[split] = ShapeViewStreamDataset(self.datasets[split])
