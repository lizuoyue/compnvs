...
class ShapeViewDataset(ShapeDataset):
    ...
    def __init__(self, 
        ...
        self.load_npz = load_npz # feed-forward mode
        ...
        if self.load_npz is not None:
            _data_per_view['npz'] = self.find_npz(self.load_npz)
    ...
    def find_depth(self):
        try:
            return self.select([sorted(glob.glob(path + '/depth/*.npz')) for path in self.paths]) # exr
        except FileNotFoundError:
    ...
    def find_npz(self, npz_folder):
        try:
            return self.select([sorted(glob.glob(path + f'/{npz_folder}*.npz')) for path in self.paths])
        except FileNotFoundError:
            raise FileNotFoundError("CANNOT find .npz files")
    ...
    def _load_view(self, packed_data, view_idx):
        ...
        voxel = None
        if packed_data.get('npz', None) is not None:
            voxel = np.load(packed_data['npz'][view_idx], allow_pickle=True)
            print('Load', packed_data['npz'][view_idx])
            voxel = {k: voxel[k] for k in voxel}

            # sid_k = os.path.basename(packed_data['npz'][view_idx]).replace('.npz', '')
            # files = glob.glob(f'ReplicaGenEncFtTriplets/easy/npz_triple/{sid_k}*')
            # if len(files) > 0:
            #     sid, k, i, j = os.path.basename(files[0]).replace('.npz', '').split('_')
            #     print(sid, k, i, j)
            #     voxel_i = np.load(f'/home/lzq/lzy/NSVF/ReplicaGenFt/all/npz/{sid}_{i}.npz')
            #     voxel_j = np.load(f'/home/lzq/lzy/NSVF/ReplicaGenFt/all/npz/{sid}_{j}.npz')
            #     for k in voxel:
            #         voxel[k] = np.concatenate([voxel[k], voxel_i[k], voxel_j[k]], axis=0)

            # voxel['vertex_input'][:, -1:] *= -1
            # voxel['vertex_input'][:, -1:] += 1.0

            # fill_mask = np.round(voxel['vertex_input'][:, -1]).astype(np.bool)
            # known_mask = ~fill_mask

            # known_ft = voxel['vertex_input'][known_mask, :-1]
            # ft_mu = known_ft.mean(axis=0)
            # ft_sigma = known_ft.std(axis=0)

            # voxel['vertex_output_norm'] = (voxel['vertex_output'] - ft_mu) / ft_sigma
            # voxel['vertex_input_norm'] = voxel['vertex_input'] * 0
            # voxel['vertex_input_norm'][fill_mask, -1] = 1
            # voxel['vertex_input_norm'][known_mask, :-1] = voxel['vertex_output_norm'][known_mask]
            # voxel['norm_mu'] = ft_mu
            # voxel['norm_sigma'] = ft_sigma
        ...
        return {
            ...
            ,
            'center_pts': voxel['center_pts'].transpose() if (voxel is not None and 'center_pts' in voxel) else None,
            'center_to_vertex': voxel['center_to_vertex'].transpose() if (voxel is not None and 'center_to_vertex' in voxel) else None,
            'center_batch': np.zeros((1, voxel['center_pts'].shape[0]), np.int) if (voxel is not None  and 'center_pts' in voxel) else None,
            'vertex_idx': voxel['vertex_idx'].transpose() if (voxel is not None and 'vertex_idx' in voxel) else None,
            'vertex_input': voxel['vertex_input'].transpose() if (voxel is not None and 'vertex_input' in voxel) else None,
            'vertex_batch': np.zeros((1, voxel['vertex_idx'].shape[0]), np.int) if (voxel is not None and 'vertex_idx' in voxel) else None,
            # 'vertex_output': voxel['vertex_output'].transpose() if (voxel is not None and 'vertex_output' in voxel) else None,
            # 'vertex_reference': voxel['reference'].reshape((3, 1)) if (voxel is not None and 'reference' in voxel and len(voxel['reference']) ) else None,
            'init_pts': voxel['pts'].transpose() if (voxel is not None and 'pts' in voxel) else None,
            'init_rgb': voxel['rgb'].transpose() if (voxel is not None and 'rgb' in voxel) else None,
            'init_batch': np.zeros((1, voxel['pts'].shape[0]), np.int) if (voxel is not None and 'pts' in voxel) else None,
            # 'vertex_mu': voxel['norm_mu'].reshape((-1, 1)) if voxel is not None else None,
            # 'vertex_sigma': voxel['norm_sigma'].reshape((-1, 1)) if voxel is not None else None,
            # 'vertex_feature_valid': vertex['vertex_feature_valid'].reshape((1, -1)) if vertex is not None else None,
            # 'geo_x': geo_x.transpose([3, 0, 1, 2]) if geo_x is not None else None,
            # 'geo_y': geo_y.transpose([3, 0, 1, 2]) if geo_y is not None else None,
            # 'geo_mask': geo_mask if geo_mask is not None else None,
            # 'info_grid': info_grid.transpose() if info_grid is not None else None,
        }
    ...
    def collater(self, samples):
        ...
        for key in samples[0][2][0]:
            ...
            elif key.startswith('voxel_') or key.startswith('vertex_') or key.startswith('center_') or key.startswith('init_'):
                if samples[0][2][0][key] is not None:
                    if key.endswith('_batch'):
                        results[key] = torch.from_numpy(
                            np.array([np.concatenate([d[key]+int(bias) for bias,d in enumerate(s[2])], axis=1) for s in samples])
                        )
                    else:
                        results[key] = torch.from_numpy(
                            np.array([np.concatenate([d[key] for d in s[2]], axis=1) for s in samples])
                        )
