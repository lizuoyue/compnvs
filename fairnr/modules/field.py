...
class RaidanceField(Field):
    ...
    def __init__(self, args):
        ...
        # input specs
        # print(args.inputs_to_density) # string "emb:6:8"
        # print(args.inputs_to_texture) # string "feat:0:256"
        ...
        # print(self.den_filters, self.den_ori_dims, self.den_input_dims)
        # OrderedDict([
        #     ('emb', NeRFPosEmbLinear(Cat(Sinusoidal (in=8, out=96, angular=False), 8)))
        # ])
        # [8]
        # [104]
        # 
        # print(self.tex_filters, self.tex_ori_dims, self.tex_input_dims)
        # OrderedDict([
        #     ('feat', Identity())
        # ])
        # [256]
        ...
        if getattr(args, "fix_field", False) is True:
    ...
    def forward(self, inputs, outputs=['sigma', 'texture']):
        ...
        if 'texture' in outputs:        
            ...
            if getattr(self.args, "use_discriminator", False):
                inputs['texture'] = torch.cat([inputs['texture'], inputs['emb']], dim=-1)
...
@register_field('rgba_radiance_field')
class RGBARadianceField(RaidanceField):

    def __init__(self, args):
        super().__init__(args)
        assert(args.voxel_embed_dim % 4 == 0)
        self.quater_emb_dim = args.voxel_embed_dim // 4
        self.rgb_posenc = True
        self.build_rgba_feature_field(args)
        self.build_rgba_density_predictor(args)
        self.build_rgba_texture_renderer(args)
    
    def build_rgba_feature_field(self, args):
        den_feat_dim = self.tex_input_dims[0]
        if self.rgb_posenc:
            indim = sum(self.den_input_dims)
        else:
            indim = sum(self.den_ori_dims)
        self.feature_field_tex = ImplicitField(
            indim, den_feat_dim, args.feature_embed_dim,
            args.feature_layers + 2 if not self.nerf_style else 8, # +2 is to adapt to old code
            with_ln=self.with_ln if not self.nerf_style else False, 
            skips=self.skips if not self.nerf_style else [4],
            spec_init=True if not self.nerf_style else False)  

    def build_rgba_density_predictor(self, args):
        den_feat_dim = self.tex_input_dims[0]
        self.predictor = SignedDistanceField(
            den_feat_dim + self.quater_emb_dim, args.density_embed_dim, recurrent=False, num_layers=1, 
            with_ln=self.with_ln if not self.nerf_style else False,
            spec_init=True if not self.nerf_style else False)

    def build_rgba_texture_renderer(self, args):
        tex_input_dim = sum(self.tex_input_dims)
        self.renderer = SingleTextureField(
            tex_input_dim + self.quater_emb_dim, args.texture_embed_dim, 
            args.texture_layers + 2 if not self.nerf_style else 2, 
            with_ln=self.with_ln if not self.nerf_style else False,
            spec_init=True if not self.nerf_style else False)
    
    def polynomial(self, pos, voxel_size):
        res = torch.remainder(pos, voxel_size)
        x = pos[:,0:1]
        y = pos[:,1:2]
        z = pos[:,2:3]
        x2 = x*x
        ...
        return torch.stack([
            x*0.0+1.0,
            x, y, z,
            0.5*x2, 0.5*y2, 0.5*z2, xy, xz, yz,
            1/6*x3, 1/6*y3, 1/6*z3, 0.5*x2y, 0.5*x2z, 0.5*y2x, 0.5*y2z, 0.5*z2x, 0.5*z2y, xyz,
        ], dim=1)
    ...
    def forward(self, inputs, outputs=['sigma', 'texture']):
        ...
        # dists torch.Size([N])
        # emb torch.Size([N, 32])
        # emb [__a__ __R__ __G__ __B__]

        # inputs['emb'] = nn.Tanh()(inputs['emb'])
        ...
        if inputs.get('feat', None) is None:        
            for i, name in enumerate(self.den_filters):
                assert(name == 'emb')
                d_in, func = self.den_ori_dims[i], self.den_filters[name]
                filtered_inputs += [func(inputs[name][:,:self.quater_emb_dim])]
            filtered_inputs = torch.cat(filtered_inputs, -1)
            if context is not None:
                if getattr(self.args, "hypernetwork", False):
                    filtered_inputs = (filtered_inputs, context)
        ...
        rgb_emb = inputs['emb'][:,self.quater_emb_dim:].reshape((-1,3,self.quater_emb_dim)) # N 3 D/4
        rgb_emb = rgb_emb.reshape((-1,self.quater_emb_dim)) # N*3 D/4
        if self.rgb_posenc:
            rgb_emb_posenc = self.den_filters['emb'](rgb_emb) # N*3 ???
            inputs['feat_rgb'] = self.feature_field_tex(*(rgb_emb_posenc, )) # N*3 256
        else:
            inputs['feat_rgb'] = self.feature_field_tex(*(rgb_emb, )) # N*3 256

        if 'sigma' in outputs:
            assert 'feat' in inputs, "feature must be pre-computed"
            inputs['sigma'] = self.predictor(
                torch.cat([
                    inputs['feat'],
                    inputs['emb'][:,:self.quater_emb_dim],
                ], dim=-1)
            )[0]
        ...
        if 'texture' in outputs:
            ...
            filtered_inputs = torch.cat([inputs['feat_rgb'], rgb_emb], dim=-1)
            inputs['texture'] = self.renderer(filtered_inputs).reshape(-1,3) # N,3
    ...
    def forward_direct(self, inputs, outputs=['sigma', 'texture']):
        # inputs
        # pos torch.Size([N, 3])
        # ray torch.Size([N, 3])
        # dists torch.Size([N])
        ...
        # pos_poly = self.polynomial(inputs['pos'] / 0.1, 1) # N, 20, 1
        # emb = inputs['emb'].reshape(-1, 4, 20) # N, 4, 20
        # res = torch.matmul(emb, pos_poly)[..., 0] # N, 4

        inputs['sigma'] = inputs['emb'][:,0]#res[:,0]
        inputs['texture'] = inputs['emb'][:,1:4]#res[:,1:]
...
@register_field('rgba_sep_radiance_field')
class RGBASepRadianceField(RaidanceField):
    ...
    def build_rgba_feature_field(self, args):
        den_feat_dim = self.tex_input_dims[0]
        self.feature_field_tex = ImplicitField(
            sum(self.den_input_dims)*3, den_feat_dim, args.feature_embed_dim,
            args.feature_layers + 2 if not self.nerf_style else 8, # +2 is to adapt to old code
            with_ln=self.with_ln if not self.nerf_style else False, 
            skips=self.skips if not self.nerf_style else [4],
            spec_init=True if not self.nerf_style else False)  
    ...
    def build_rgba_texture_renderer(self, args):
        tex_input_dim = sum(self.tex_input_dims)
        self.renderer = TextureField(
            tex_input_dim + self.quater_emb_dim*3, args.texture_embed_dim, 
            args.texture_layers + 2 if not self.nerf_style else 2, 
            with_ln=self.with_ln if not self.nerf_style else False,
            spec_init=True if not self.nerf_style else False)
    ...
    def forward(self, inputs, outputs=['sigma', 'texture']):
        ...
        rgb_emb = rgb_emb.reshape((-1,self.quater_emb_dim)) # N*3 D/4
        rgb_emb_posenc = self.den_filters['emb'](rgb_emb) # N*3 ???
        rgb_emb_posenc = rgb_emb_posenc.reshape((-1, 3 * rgb_emb_posenc.shape[-1])) # N !!!

        inputs['feat_rgb'] = self.feature_field_tex(*(rgb_emb_posenc, )) # N 256
        ...
        if 'texture' in outputs:
            ...
            filtered_inputs = torch.cat([inputs['feat_rgb'], inputs['emb'][:,self.quater_emb_dim:]], dim=-1)
            inputs['texture'] = self.renderer(filtered_inputs)
